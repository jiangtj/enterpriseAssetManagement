                        ...
                        <div class="ibox-content">
                            <form role="form" class="form-inline">
                                ...
                                <div class="btn-toolbar pull-right" role="toolbar">
                                    <div class="btn-group">
                                        <button @click="showAddModal()"  v-if="PermissionName('point:add')" class="btn btn-outline btn-primary" type="button">新增</button>
                                        <button @click="showUpdateModal(tableSelectData[0])" v-if="hasOneChecked && PermissionName('point:update')" class="btn btn-outline btn-primary" type="button">修改</button>
                                        <button @click="deleteAll()" v-if="hasChecked && PermissionName('point:delete')" class="btn btn-outline btn-danger" type="button">删除</button>
                                    </div>
                                    <div class="btn-group">
                            ...
                            <div class="table-responsive">
                                <tt-table v-bind:data="tableData" :selection = "true" v-model="tableSelectData">
                                    <template slot="tt-body-operation" scope="props">
                                        <button @click="showUpdateModal(props.row)" v-if="PermissionName('point:update')" class="btn btn-table btn-primary btn-rounded" type="button">修改</button>
                                    </template>
                                </tt-table>
                            </div>
                            <!-- 分页 -->
    ...
    RouteConfig.deploy({
        ...
        methods: {
            ...
            getTableList:function () {
                let self = this;
                Server.point.getList.setData(self.conditions).post(data => {
            ...
            deleteAll:function () {
                ...
                SweetAlertUtils.show().sure(function () {
                    ...
                    Server.point.delete.setData("ids="+ids).post(() => self.getTableList());
                });
            },
